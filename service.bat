...
:: SCP:SL entry points are small wrappers around the shared HARD profile runner.
:: Resolve the requested profile and export its variables before parsing the
:: runner, so Install Service works for SCP-SL.bat, ALT and all HARD wrappers.
set "parseFile=!selectedFile!"
set "scpslProfile="
for /f "delims=" %%P in ('powershell -NoProfile -Command "$content = Get-Content -Raw -LiteralPath '!selectedFile!'; $match = [regex]::Match($content, 'run scpsl hard\.bat\S*\s+(\d{1,2})', [System.Text.RegularExpressions.RegexOptions]::IgnoreCase); if ($match.Success) { $match.Groups[1].Value }"') do set "scpslProfile=%%P"

if defined scpslProfile (
    call "%~dp0utils\run scpsl hard.bat" "!scpslProfile!" "!selectedFile!" service_export
    if errorlevel 1 (
        echo Failed to load SCP:SL HARD profile !scpslProfile!.
        pause
        goto menu
    )
    set "parseFile=%~dp0utils\run scpsl hard.bat"
)
...
for /f "tokens=*" %%a in ('type "!parseFile!"') do (
    set "line=%%a"
